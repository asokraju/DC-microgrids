...
class Buck_microgrid_v0(gym.Env):
    ...
    def __init__(self, dt = 5e-6):
        super(Buck_microgrid_v0, self).__init__()
        ...
        self.action_space = spaces.Box(low=np.array([0.0,0.0,0.0,0.0]), high=np.array([1., 1., 1., 1.], dtype=np.float64))
    ...
    def _get_state(self):
        ...
        It = self.Itdes#np.random.uniform(low = self.Itdes-1, high = self.Itdes+1)
...
class Buck_microgrid_v1(gym.Env):
    ...
    def __init__(self, dt = 5e-6):
        super(Buck_microgrid_v1, self).__init__()
        ...
        self.R_b = np.matmul(np.matmul(self.inc_mat, np.linalg.inv(self.Rt)), self.inc_mat.T)
        #step size; since L and C are very low, the ode becomes stiff
        #For the default parameters the step size should in the order of 1e-6
        self.T = dt
        ...
        low_obs = np.full(shape = (8,), fill_value = -np.inf, dtype=np.float64)
        high_obs = np.full(shape = (8,), fill_value = np.inf, dtype=np.float64)
    ...
    def desired(self):
        #Provides the steady-state variables
        return np.array([self.Ides, self.Vdes, self.udes, self.action_des])
    ...
    def step(self, action):
        ...
        self.state = np.concatenate([new_i, new_v]).astype(np.float32)
    ...
    def plot(self, savefig_filename=None):
        ...
        state_dim = self.observation_space.shape[0]
        act_dim = self.action_space.shape[0]
        des1 = np.array([self.Ides, self.Vdes, self.action_des])
        title_nodes = ['Node - 1', 'Node - 2', 'Node - 3', 'Node - 4']
        title_state = ['DGU current','Load Voltage', 'action']
        test_steps = self.count_steps
        time = np.array(range(test_steps), dtype=np.float32)*self.T
        test_obs_reshape = np.concatenate(self.state_trajectory).reshape((test_steps ,self.observation_space.shape[0]))
        #print(np.shape(self.action_trajectory),self.action_trajectory)
...
class Buck_microgrid_dist_v0(gym.Env):
    """
    Buck converter model following gym interface
    We are assuming that the switching frequency is very High
    Action space is continious
